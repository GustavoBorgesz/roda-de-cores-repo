...
function gradient(){let c=paletteValues(),css='linear-gradient(135deg, '+c.join(', ')+')';openTool('Gradiente automático');let body=$('#toolModalBody');body.innerHTML='<div class="gradient-preview"></div><div class="gradient-value" aria-label="Valor CSS do gradiente">'+css+'</div><button class="action-btn png-btn" id="copyGradient" style="margin-top:10px;width:100%">Copiar CSS</button>';body.querySelector('.gradient-preview').style.background='linear-gradient(135deg,'+c.join(',')+')';body.querySelector('#copyGradient').onclick=()=>{copy(css);show('Gradiente copiado!')}}
