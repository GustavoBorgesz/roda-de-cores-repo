...
const drop=$('#drop'),input=$('#image'),canvas=$('#canvas'),imageColors=$('#imageColors');
function imageFile(f){if(!f?.type.startsWith('image/'))return show('Escolha uma imagem');let rd=new FileReader();rd.onload=()=>{let im=new Image();im.onload=()=>extract(im);im.src=rd.result};rd.readAsDataURL(f)}
...
function gradient(){let c=paletteValues();openTool('Gradiente automático');let body=$('#toolModalBody');body.innerHTML='<div class="gradient-preview"></div><code>linear-gradient(135deg, '+c.join(', ')+')</code>';body.querySelector('.gradient-preview').style.background='linear-gradient(135deg,'+c.join(',')+')';copy('linear-gradient(135deg, '+c.join(', ')+')');show('Gradiente copiado!')}
