import { harmonies, norm, hsl, rgb, toHex, hexToHsl, colorName, ratio, simulateBlindness } from './colorMath.js';
function $(s) {
...
function snapshot() { return { h: hue, s: saturation, l: lightness, m: mode }; }
function applySnapshot(x) { hue = x.h; saturation = x.s; lightness = x.l; mode = x.m; }
function push() {
...
function drawHarmonyOverlay() {
    ...
    if (points.length === 2) {
        ...
        line.setAttribute('stroke', 'rgba(13,15,20,.85)');
        ...
        shape.setAttribute('stroke', 'rgba(13,15,20,.85)');
    ...
    points.forEach((p, i) => {
        ...
        dot.setAttribute('stroke', 'rgba(13,15,20,.85)');
...
function colors() {
    if (mode === 'monochromatic')
        return [22, 38, 55, 72, 88].map(l => ({ h: hue, s: saturation, l }));
...
function render() {
    sat.value = String(saturation);
    light.value = String(lightness);
...
sat.oninput = (e) => { saturation = +e.target.value; render(); };
light.oninput = (e) => { lightness = +e.target.value; render(); };
$('#apply').onclick = () => {
...
hex.onkeydown = (e) => { if (e.key === 'Enter')
    $('#apply').click(); };
...
$('#copyPalette').onclick = () => copy(paletteValues().join(', '));
$('#save').onclick = save;
$('#theme').onclick = () => {
...
$$('.tab').forEach(b => b.onclick = () => {
    ...
    $('#wheelTab').hidden = b.dataset.tab !== 'wheel';
    $('#imageTab').hidden = b.dataset.tab !== 'image';
});
...
drop.ondrop = (e) => { e.preventDefault(); drop.classList.remove('drag'); imageFile(e.dataTransfer?.files[0]); };
input.onchange = () => imageFile(input.files?.[0]);
$('#favorites').onclick = () => { $('#drawer').hidden = false; $('#scrim').hidden = false; };
...
function closeOverlays() { closeDrawer(); closeTool(); }
...
function exportPNG() {
    ...
    can.toBlob(b => {
        if (b) {
            downloadBlob('chromatica-palette.png', b);
            show('Paleta baixada em PNG!');
        }
        else
            show('Não foi possível gerar o PNG');
    }, 'image/png');
}
function gradient() {
    ...
    body.querySelector('.gradient-preview').style.background = 'linear-gradient(135deg,' + c.join(',') + ')';
    body.querySelector('#copyGradient').onclick = () => { copy(css); show('Gradiente copiado!'); };
}
function contrast() {
    const c = paletteValues();
...
function share() {
    ...
    if (navigator.share)
        navigator.share({ title: 'Chromatica Palette', url: u }).catch(() => { });
    else
        copy(u);
...
$('#exportCss').onclick = exportCSS;
$('#exportPng').onclick = exportPNG;
$('#gradientBtn').onclick = gradient;
...
function simulateBlind() {
    ...
    const labels = { protanopia: 'Protanopia', deuteranopia: 'Deuteranopia', tritanopia: 'Tritanopia', achromatopsia: 'Acromatopsia' };
...
$('#previewBtn').onclick = uiPreview;
$('#blindBtn').onclick = simulateBlind;
render();
renderSaved();
loadShared();
